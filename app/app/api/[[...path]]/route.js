...
async function getSessionUser(request) {
  // Minimal session via cookie set by NextAuth. We'll also accept x-user-id header for tests.
  const cookie = request.headers.get('cookie') || ''
  const match = cookie.match(/next-auth.session-token=([^;]+)/) || cookie.match(/__Secure-next-auth.session-token=([^;]+)/)
  const userIdHeader = request.headers.get('x-user-id')
  const db = await getDb()
  if (userIdHeader) {
    const u = await db.collection('users').findOne({ id: userIdHeader })
    if (u) return { id: u.id, email: u.email, role: u.role, name: u.name }
  }
  if (!match) return null
  // We will not decode JWT here; use a lightweight lookup cache table where we map session token to user id on login
  const token = decodeURIComponent(match[1])
  const mapping = await db.collection('auth_sessions').findOne({ token })
  if (!mapping) return null
  const user = await db.collection('users').findOne({ id: mapping.user_id })
  if (!user) return null
...
export async function GET(request, { params }) {
  try {
    ...
    if (pathname === '/traders') {
      ...
      if (investorId) {
        // fetch accepted openings to unmask selectively
...
export async function POST(request, { params }) {
  try {
    ...
    if (pathname === '/contact/request') {
      ...
      // Rate limit by counting requests in last 1h
      ...
      await db.collection('contact_credits').updateOne({ user_id: me.id }, { $inc: { credits_used: 1 } })
    ...
    if (pathname === '/contact/decision') {
      ...
      // ensure this trader owns the profile
    ...
    if (pathname === '/auth/session-map') {
      // Called post successful NextAuth signIn to map session token -> user id for masking logic
      const { token } = await request.json()
      if (!token) return json({ ok: false })
      const user = await getSessionUser(request)
      // If NextAuth set cookie, getSessionUser may already resolve; fallback to explicit body userId (not recommended)
      if (!user) return json({ ok: false })
      const db = await getDb()
      await db.collection('auth_sessions').updateOne({ token }, { $set: { token, user_id: user.id, at: new Date() } }, { upsert: true })
      return json({ ok: true })
    }
